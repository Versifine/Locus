...
| 开发策略 | Bot 模式为核心 | Proxy 已完成使命（协议学习/调试），归档保留；Bot 直连服务器 |
| 配置格式 | YAML | 人类友好 |
...
| ID | 功能 | 描述 | 里程碑 |
|----|------|------|--------|
| F01 | TCP 代理 | 客户端 ↔ Locus ↔ 服务器，透明转发 | v0.1 ✅ (已归档) |
| F02 | 配置文件 | YAML 配置 | v0.1 ✅ |
| F03 | 协议解析 | 解析 VarInt、Packet 结构（共享层） | v0.2 ✅ |
| F04 | 握手处理 | 解析 Handshake、Login 流程 | v0.2 ✅ |
| F05 | 聊天拦截 | 捕获聊天消息，触发事件 | v0.3 ✅ |
| F06 | LLM 集成 | 调用 DeepSeek API | v0.3 ✅ |
| F07 | 聊天回复 | AI 生成回复并发送到游戏 | v0.3 ✅ |
| F08 | Bot 登录 | Locus 作为独立客户端登录服务器（Headless Bot） | v0.4 |
| F09 | Bot 保活 | Keep-Alive 应答 + 位置同步确认 | v0.4 |
...
| v0.3 | 聊天拦截 + LLM 集成（Proxy 模式） | 第 6-7 周 | ✅ |
| **v0.4** | **Headless Bot 登录 + 保活 + 聊天** | **第 8-10 周** | **⬜** |
| v0.5 | 世界感知 + 视野系统 | 第 11-16 周 | ⬜ |
| v0.6 | 原子动作 + 状态反馈 | 第 17-24 周 | ⬜ |

> **v0.4 是架构转折点**：从 Proxy 寄生模式切换到 Bot 直连模式。
> Proxy 代码归档保留（`internal/proxy/`），不再维护，仅用于协议调试。
...
│  ┌────────────────────────────────────────────────────────┐  │
│  │              Protocol Layer (协议层)                    │  │
│  │   VarInt / Packet / Handshake / Login / Chat / NBT     │  │
│  └───────────────────────┬────────────────────────────────┘  │
│                          ▼                                    │
│  ┌────────────────────────────────────────────────────────┐  │
│  │              Headless Bot (身体)                        │  │
│  │   独立 TCP 连接 → MC 服务器（离线模式）                   │  │
│  │   登录 / Keep-Alive / 收发包 / 位置同步                  │  │
│  └───────────────────────┬────────────────────────────────┘  │
│                          ▼                                    │
│  ┌────────────────────────────────────────────────────────┐  │
│  │              EventBus (事件总线)                        │  │
│  │   ChatEvent / (未来: PositionEvent, EntityEvent ...)    │  │
│  └───────────────────────┬────────────────────────────────┘  │
│                          ▼                                    │
│  ┌────────────────────────────────────────────────────────┐  │
│  │              Agent (决策层)                             │  │
...
│  ┌────────────────────────────────────────────────────────┐  │
│  │              LLM Layer                                 │  │
│  │           DeepSeek / Qwen / GPT (可切换)                │  │
│  └────────────────────────────────────────────────────────┘  │
│                                                               │
│  ┌ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ┐ │
│  │            未来层级 (v0.5+)                              │ │
│  │  World State → Body Layer → Skills → Cognition          │ │
│  └ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ┘ │
│                                                               │
│  ┌ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ┐ │
│  │            Proxy (已归档，仅用于协议调试)                  │ │
...
| 风险 | 影响 | 对策 |
|------|------|------|
| MC 协议复杂 | 开发慢 | 参考 minecraft.wiki / minecraft-data，只实现必要包 |
...
| **2026-02-07** | **v0.3 完成：聊天拦截 + LLM 集成 + 回复注入全流程闭环** |
| 2026-02-07 | 架构决策：Proxy 归档，Bot 直连模式为核心 |
| 2026-02-07 | 系统架构图重绘：移除 Proxy/Bot 双路径，Bot 为唯一主路径 |
| 2026-02-07 | 开发策略从"代理模式 → Bot 模式"更新为"Bot 模式为核心" |
| 2026-02-07 | 规划 v0.4：Headless Bot 登录 + Keep-Alive + 聊天 |
